...
public class Matrices {
    ...
    public static class Vector {
        ...
        public Vector add(Vector other) {
            //Векторы разной длины нельзя сложить, поэтому бросаем исключение
            if (other.items.length != this.items.length) {
                throw new IllegalArgumentException();
            }
            //Создаем вектор в который будем записывать сумму двух векторов
            Vector result = new Vector(this.items.length);
            //Складываем каждый элемент вектора this(то есть текущего объекта вектора) с
            //каждым элементом вектора other и записываем это результирующий вектор
            for (int i = 0; i < this.items.length; i++) {
                result.items[i] = this.items[i] + other.items[i];
            }
            //Возвращаем result, в котором записана сумма векторов
            return result;
        }
